const main = async () => {
  ...
  // mint the same domain again => reverts with AlreadyRegistered custom error
  txn = await domainContract.register("a16z", {
    value: hre.ethers.utils.parseEther("1234"),
  });
  await txn.wait();

  // mint a domain with an invalid name => reverts with InvalidName custom error
  txn = await domainContract.register("a1", {
    value: hre.ethers.utils.parseEther("1234"),
  });
  await txn.wait();
