...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("SAMS Name Service", "SAM") {
        tld = _tld;
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 5 * 10**16; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.05 Matic cause the faucets don't give a lot
        } else if (len == 4) {
    ...
    function register(string calldata name) public payable {
        // require(domains[name] == address(0), "Domain not available!");
        ...
        // console.log("Final finalSvg:", finalSvg);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Sams name service", "image": "data:image/svg+xml;base64,',
    ...
    function setRecord(string calldata name, string calldata record) public {
        // require(
        //     domains[name] == msg.sender,
        //     "Not Authorized to change the record!"
        // );
